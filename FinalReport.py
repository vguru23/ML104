...
""")
    ...
    This is why in our next steps we aim to add more data to capture the onboard/offboard period and maybe include users onboarding and offboarding toilet seat multiple times to lessen sitting class and increase transitional classes. In addition to that, we want to explore other advanced machine learning algorithms, such as deep learning models like LSTMs or CNNs. They could provide deeper insights into sequential or spatial data patterns. Another area of improvement is real-time implementation—building a system that deploys the model in a live environment to classify behaviors dynamically.""")
